...
const ALERT_LINK: Record<string, string> = {
  out_of_range: "/admin/attendance?status=late",
...
export default async function AdminHome() {
  ...
  // Count on a specific column, not *. Column-level grants keep qr_secret out
  // of reach, and a select * on rooms fails outright rather than omitting it.
    ...
    <>
      ...
      <section className="mt-10">
              ...
              return (
                <li key={i}>
                  <Link
                    ...
                    className="block rounded-lg border bg-white p-4 transition-colors hover:border-[#0B6E5F]"
        ...
        <p className="mt-4 text-xs text-[#5A6B7A] leading-relaxed">
          Covers today only: scans from outside a laboratory, accounts used on
          another phone, arrivals more than 45 minutes late, classes that were
          never opened, and classes left open after they ended.
        </p>
      </section>
    </>
  );
