...
const ALERT_LINK: Record<string, string> = {
  ...
  streak: "/admin/at-risk",
};

const ALERT_LINK_LABEL: Record<string, string> = {
  out_of_range: "View attendance",
  device_mismatch: "View students",
  very_late: "View late arrivals",
  not_started: "View sections",
  left_open: "View sections",
  late_open: "View sections",
  streak: "View at-risk students",
};
                ...
                >
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1">
                      {a.actor_id && (
                        <Link
                          href={`/admin/users/${a.actor_id}`}
                          className="text-xs font-medium text-[#0B6E5F] underline underline-offset-4 hover:opacity-70"
                        >
                          View {a.actor_role === "teacher" ? "teacher" : "student"} profile →
                        </Link>
                      )}
                      <Link
                        ...
                        className="text-xs text-[#5A6B7A] underline underline-offset-4 hover:text-[#0B6E5F]"
                      >
                        {ALERT_LINK_LABEL[a.kind] ?? "View details"} →
                      </Link>
                    </div>
                  </div>
                </li>
